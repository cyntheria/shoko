...
impl<'a> ShokoReader<'a> {
    ...
    pub fn read_blob(&mut self, offset: u64, size: u64, clevel: u8) -> io::Result<Vec<u8>> {
        ...
        if clevel > 0 {
            decompress(&buffer).map_err(|e| io::Error::new(io::ErrorKind::InvalidData, e))
        } else {
            Ok(buffer)
        }
    }
    ...
    pub fn get_footer_info(&mut self) -> io::Result<(u64, u32)> {
        ...
        if file_len < 14 {
            return Err(io::Error::new(io::ErrorKind::InvalidInput, "File too small for Shoko archive"));
        }
        ...
        if &magic != b"SK" {
            return Err(io::Error::new(io::ErrorKind::InvalidData, "Invalid Shoko magic trailer"));
        }
